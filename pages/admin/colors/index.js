...
export default function Colors() {
	...
	async function saveColor(ev) {
		...
		if (!editedColor) {
			...
			await axios.put(`/api/colors/${editedColor._id}`, data);
		}
	...
	async function deleteColor(_id) {
		await axios.delete('/api/colors/' + _id);
		setDeletedCount((cnt) => cnt + 1);
	}
	...
	return (
		<>
			<Head>
				<title>Open Fashion | Colors</title>
			</Head>
			<AdminLayout>
				<Content>
					...
					<form className="" onSubmit={saveColor}>
						<label className="block mb-1">
							{editedColor ? (
								<>
									<span>Edit </span>
									<span>{editedColor.name}</span>
								</>
							) : (
								'New color'
							)}
